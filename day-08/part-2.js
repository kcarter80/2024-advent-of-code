    ...
    Object.entries(antennas).forEach(([antenna, coords]) => {
        for (let i = 0; i < coords.length; i++) {
            for (let ii = i + 1; ii < coords.length; ii++) {
                // there are always antinodes at the antenna locations
                antinodes.add(`${coords[i][0]},${coords[i][1]}`)
                antinodes.add(`${coords[ii][0]},${coords[ii][1]}`)
                ...
                while (antinode1[0] >= 0 && antinode1[0] <= maxX && antinode1[1] >= 0 && antinode1[1] <= maxY) {
                    //console.log(`an1: (${antinode1[0]}, ${antinode1[1]})`)
                    antinodes.add(`${antinode1[0]},${antinode1[1]}`)
                    antinode1[0] += xDistance
                    antinode1[1] += yDistance
                }
                while (antinode2[0] >= 0 && antinode2[0] <= maxX && antinode2[1] >= 0 && antinode2[1] <= maxY) {
                    //console.log(`an2: (${antinode2[0]}, ${antinode2[1]})`)
                    antinodes.add(`${antinode2[0]},${antinode2[1]}`)
                    antinode2[0] -= xDistance
                    antinode2[1] -= yDistance
                }
            }
        }
    })
